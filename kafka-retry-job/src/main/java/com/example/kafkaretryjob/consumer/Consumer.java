package com.example.kafkaretryjob.consumer;

import com.example.kafkaretryjob.publisher.KafkaPublisher;
...
public class Consumer {
    ...
    public Consumer(KafkaPublisher kafkaPublisher) {
    ...
    @KafkaListener(
            topics = {ORDER_CREATE_KAFKA_CONSUMER_ERROR},
            groupId = GROUP_ID,
    ...
    private boolean isRetryable(String errorTopic, int currentRetryCount) {
        int retryCountLimit = retryCountsByTopicNames.getOrDefault(errorTopic, 0);
        boolean limitless = retryCountLimit == 0;
        return limitless || currentRetryCount < retryCountLimit;
    }

    private int getRetryCount(JsonNode jsonNode) {
